...
export default function RawabetResultModal({
  ...
  return (
    ...
    >
      ...
      {/* Modal */}
      <div className="relative w-full max-w-sm mx-4 mb-4 sm:mb-0 bg-[#1C1A2E] rounded-2xl border border-[#2E2A45] overflow-hidden shadow-2xl animate-slide-up">
        {/* Header */}
        <div className="px-5 pt-5 pb-3 text-center">
          ...
          <p className="text-sm text-[#7A7589] mt-1">روابط #{puzzleNumber}</p>
        </div>
        ...
        <div className="px-5 pb-3 flex items-center justify-center gap-2">
          <span className="text-xs text-[#7A7589]">الأخطاء:</span>
          {Array.from({ length: 4 }).map((_, i) => (
            <div
              key={i}
              className={`w-3 h-3 rounded-full ${i < mistakes ? "bg-[#E8604C]" : "bg-[#2E2A45]"}`}
            />
          ))}
        </div>

        {/* Stats */}
        <div className="mx-5 mb-3 grid grid-cols-3 gap-2 bg-[#141218] rounded-xl p-3">
          {[
            ...
            <div key={label} className="flex flex-col items-center gap-0.5">
              ...
              <span className="text-[10px] text-[#7A7589] text-center leading-tight">{label}</span>
            </div>
          ))}
        </div>
        ...
        <div className="px-5 pb-5 flex gap-2">
          ...
          <div className="flex flex-col items-center justify-center bg-[#141218] rounded-xl px-3 py-2 min-w-[72px]">
            <span className="text-[10px] text-[#7A7589]">اللغز القادم</span>
