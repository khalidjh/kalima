...
export default function NahlaPage() {
  ...
  // Hexagon positions (center + 6 around)
  const hexPositions = [
    { x: 50, y: 50 }, // center
    { x: 50, y: 15 }, // top
    { x: 85, y: 32 }, // top-right
    { x: 85, y: 68 }, // bottom-right
    { x: 50, y: 85 }, // bottom
    { x: 15, y: 68 }, // bottom-left
    { x: 15, y: 32 }, // top-left
  ];
  ...
  return (
    <main className="min-h-screen bg-[#0F0C00] text-white flex flex-col items-center px-4 py-4" dir="rtl">
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-surface border border-border rounded-2xl p-4 mb-4">
          ...
          <div className="w-full h-2 bg-border rounded-full overflow-hidden">
            <div
              ...
              style={{ width: `${Math.min(rank.pct, 100)}%` }}
            />
          </div>
        ...
        {/* Toast */}
        {toast && (
          <div className="text-center mb-3 animate-toast">
            <span className="bg-surface border border-border text-sm font-semibold px-4 py-2 rounded-xl inline-block">
              {toast}
            </span>
          </div>
        )}
        ...
        <div className="flex items-center justify-center gap-3 mb-6">
          <button
            ...
            className="bg-surface border border-border rounded-xl px-5 py-3 font-bold text-sm hover:border-primary/40 transition-colors"
          >
            حذف
          </button>
          <button
          ...
          <button
            ...
            className="bg-primary text-[#0A0A0A] rounded-xl px-5 py-3 font-bold text-sm hover:opacity-90 transition-opacity"
          >
            إدخال
          </button>
        </div>
        ...
        <button
          onClick={handleShare}
          className="w-full bg-surface border border-border hover:border-primary/40 rounded-xl py-3 font-bold text-sm transition-colors mb-4"
        >
